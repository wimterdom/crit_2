...
def count_reset():
    ...
    df=df[['voter_id','Name','Gender','City','Email','Passw','hasVoted']]
...
def reset_voter_list():
    df = pd.DataFrame(columns=['voter_id','Name','Gender','City','Email','Passw','hasVoted'])
    df=df[['voter_id','Name','Gender','City','Email','Passw','hasVoted']]
    df.to_csv(path/'voterList.csv')
...
def taking_data_voter(name,gender,city,email,passw):
    df=pd.read_csv(path/'voterList.csv')
    df=df[['voter_id','Name','Gender','City','Email','Passw','hasVoted']]
    row,col=df.shape
    if row==0:
