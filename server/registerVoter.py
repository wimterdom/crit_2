...
def reg_server(root,frame1,name,sex,city,email,passw):
    if(passw=='' or passw==' '):
        msg = Message(frame1, text="Error: Missing Fileds", width=500)
        msg.grid(row = 10, column = 0, columnspan = 5)
        return -1

    vid = df.taking_data_voter(name, sex, city, email, passw)
...
def Register(root,frame1):
    ...
    Label(frame1, text="City:           ", anchor="e", justify=LEFT).grid(row = 5,column = 0)
    Label(frame1, text="Email:             ", anchor="e", justify=LEFT).grid(row = 6,column = 0)
    ...
    e5 = Entry(frame1, textvariable = city).grid(row = 5, column = 2)
    e6 = Entry(frame1, textvariable = email).grid(row = 6, column = 2)
    ...
    reg = Button(frame1, text="Register", command = lambda: reg_server(root, frame1, name.get(), sex.get(), city.get(), email.get(), password.get()), width=10)
